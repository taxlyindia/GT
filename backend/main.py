...
# SEC-04 FIX: Add security headers to all responses
...
if _is_wildcard:
    # Wildcard mode — Bearer-token auth still works; cookies/credentials blocked by browser
    ALLOW_ORIGINS       = ["*"]
    ALLOW_CREDENTIALS   = False
else:
    # Explicit origins — include dev origins so local testing always works
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["X-Action"],   # let frontend read X-Action redirect hints
)
