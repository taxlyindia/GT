...
async def create_cash_entry(
    ...
    # BUG-CASH-01 FIX: reject zero or negative amounts
    if body.amount <= Decimal("0"):
        raise HTTPException(status_code=400, detail="Amount must be greater than zero.")
    # BUG-CASH-04 FIX: viewers cannot create entries
    if payload.get("role") == "viewer":
        raise HTTPException(status_code=403, detail="Viewers cannot modify cash entries.")
...
async def list_cash_entries(
    ...
    # BUG-CASH-02 FIX: compute running balance so cashbook ledger is accurate
    rows = []
    running = Decimal("0")
    for e in reversed(entries):  # oldest first for running total
        t = e.entry_type.value if hasattr(e.entry_type, "value") else str(e.entry_type)
        if t in ("cash_in", "bank_in"):
            running += e.amount
        elif t in ("cash_out", "cash_to_bank"):
            running -= e.amount
        rows.append({
            "id":              e.id,
            ...
            "running_balance": float(max(Decimal("0"), running)),
        })
    return list(reversed(rows))  # back to newest-first for UI
...
async def delete_cash_entry(
    ...
    # Viewer guard — consistent with edit_cash_entry above
    if payload.get("role") == "viewer":
        raise HTTPException(status_code=403, detail="Viewers cannot delete cash entries.")
    tenant_id = payload["tenant_id"]
    entry     = await db.get(CashEntry, entry_id)
    if not entry or entry.tenant_id != tenant_id:
        raise HTTPException(status_code=404, detail="Cash entry not found")
