...
async def export_fifo_excel(
    ...
    for stock in stocks:
        ...
        for t in txns:
            ...
            # Skip edit-delta adjustment records (legacy — cause double-treatment)
            ...
            if (_rtype.lower() == 'adjustment' and (
                    _rsn.startswith('Edit —') or
                    _rsn.startswith('Edit Reversal') or
                    _rsn.startswith('Edit -'))):
                continue
            ...
            if qty > 0:
                ...
                # Only add to fifo_batches if real lot (lot_remaining not None)
                # Cancellation reversal records (lot_remaining=None) restore
                # the original lots — they must NOT become new FIFO lots
                if t.lot_remaining is not None:
                    # Use lot_remaining (actual available stock) not qty (original amount)
                    lot_rem = t.lot_remaining if t.lot_remaining is not None else qty
                    fifo_batches.append({"qty_remaining": lot_rem, "purchase_rate": rate})
        ...
        # Use FIFO batch sum — qty_in - qty_out overcounts when legacy edit-delta records exist
