...
class DemoSignupRequest(BaseModel):
    ...
    email:        Optional[str]  = None   # optional — stored for email notifications
...
async def demo_signup(body: DemoSignupRequest, db: AsyncSession = Depends(get_db)):
    ...
    """
    # Validate mobile format (backend defense in depth — frontend also validates)
    if not re.match(r"^\d{10}$", body.mobile):
    ...
    user = User(
        ...
        username=body.name,
        mobile=body.mobile,
        email=body.email,            # store email for notifications
