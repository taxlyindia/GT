...
def _parse_reason(reason: str) -> tuple:
    ...
    Returns (kind, lookup_key):
      ('supplier_invno', 'GO/2025-2')    — purchase IN-lot
      ('sale_id', 5)                     — sale (including [Edited])
      ('sale_cancelled_id', 5)           — sale cancellation
      ('purchase_cancelled', 'INV-01')   — purchase cancellation
      ('purchase_edited', 'INV-01')      — purchase edit (in-place, updated reason)
      ('purchase_adj', None)             — manual stock-in adjustment
    ...
    # Purchase — Supplier Invoice {no}  (create, [Edited], [CANCELLED])
    m = re.match(r'Purchase\s*[—\-]+\s*Supplier Invoice\s+(.+?)(?:\s*\[.*\])?$', r, re.IGNORECASE)
    if m:
        invno = m.group(1).strip()
        if '[CANCELLED]' in r:
            return ('purchase_cancelled_lot', invno)   # original lot (now zeroed)
        if '[Edited]' in r:
            return ('purchase_edited', invno)
    ...
    # Sale Cancelled — Invoice ID {id}  (new cancel format)
    m = re.match(r'Sale Cancelled\s*[—\-]+\s*Invoice ID\s+(\d+)', r, re.IGNORECASE)
    if m:
        return ('sale_cancelled_id', int(m.group(1)))

    # Purchase Cancelled — Supplier Invoice {no}  (new cancel format)
    m = re.match(r'Purchase Cancelled\s*[—\-]+\s*Supplier Invoice\s+(.+)', r, re.IGNORECASE)
    if m:
        return ('purchase_cancelled', m.group(1).strip())

    # Cancellation — Supplier Invoice {no}  (old cancel format from fifo_reverse_purchase)
    m = re.match(r'Cancellation\s*[—\-]+\s*Supplier Invoice\s+(.+)', r, re.IGNORECASE)
    if m:
        return ('purchase_cancelled', m.group(1).strip())
    ...
    if m:
        ...
        if '[CANCELLED]' in r:
            return ('sale_cancelled_id', inv_id)
        return ('sale_id', inv_id)

    # Sale — {invoice_no}  (old format)
    m = re.match(r'Sale\s*[—\-]+\s*(.+)', r, re.IGNORECASE)
    if m:
        val = m.group(1).strip()
        if '[CANCELLED]' not in val:
            return ('sale_invno', val)

    # Cancelled — Invoice {id or no}  (very old format)
    ...
    # Edit Reversal — Invoice {id}  (legacy - no longer created but handle old data)
    m = re.match(r'Edit Reversal\s*[—\-]+\s*Invoice\s+(\d+)', r, re.IGNORECASE)
    if m:
        return ('sale_cancelled_id', int(m.group(1)))

    # Manual purchase stock-in
...
def _dedup_txns(all_txns: list) -> list:
    """
    Deduplicate StockTransactions so each logical event appears once.

    With in-place edits (fifo_edit_sale / fifo_edit_purchase_lot), the original
    transaction IS updated — no duplicate records, dedup is straightforward.

    Cancellation transactions are ALWAYS kept separate from original transactions.
    They represent distinct events (original sale + its reversal), both must show.

    Edit-adjustment delta records (reason starts with 'Edit —') are skipped entirely.
    These are legacy records from old edit logic that cause double-treatment.
    """
    seen: dict = {}
    for t in all_txns:
        ...
        # Cancellation txns must always be unique — never merged with original
        if kind in ('sale_cancelled_id', 'purchase_cancelled'):
            key = ('cancel', t.id)
        elif t.invoice_id is not None:
            key = ('inv', t.invoice_id, direction)
        elif kind in ('none', 'purchase_adj', 'purchase_cancelled_lot', 'purchase_edited') or lookup_key is None:
            key = ('unique', t.id)
        else:
...
def _is_edit_adjustment(txn) -> bool:
    """
    Returns True for edit-delta adjustment records from old edit logic.

    Old purchase edit logic created delta records:
      reason = "Edit — Supplier Invoice {no} (old qty=100 rate=13200)"
    These records are NOT real stock movements. The original lot is already
    mutated in-place to final values. Counting both = double-treatment.

    New edit logic (fifo_edit_purchase_lot, fifo_edit_sale) never creates
    these delta records, so this returns False for all new records.
    """
...
async def fifo_report(
    ...
    for stock in stocks:
        ...
        for t in txns:
            # Skip edit-delta adjustment records (legacy from old edit logic).
            # These records cause double-treatment: the original lot is already
            # mutated in-place to final values, so counting the delta record
            # inflates qty_out and gives a wrong (negative) qty_on_hand.
            ...
            if kind == 'supplier_invno':
                ...
                # Sale cancellation — direct reversal of original sale
                if isinstance(lkey, int):
                    orig = inv_by_id.get(lkey)
                else:
                    orig = inv_by_no.get(str(lkey))
                inv_no = orig.invoice_no if orig else (f"INV-{lkey}" if isinstance(lkey, int) else str(lkey))
                party  = (orig.customer_name or orig.customer_mobile) if orig else reason
                txn_label = "Sale Cancelled"

            elif kind == 'purchase_cancelled':
                # Purchase cancellation — direct reversal of original purchase
                sinv = sinv_by_no.get(lkey)
                if sinv is None:
                    sinv = next((s for s in all_sinvs if s.invoice_no and lkey in s.invoice_no), None)
                inv_no = sinv.invoice_no if sinv else str(lkey)
                party  = (sinv.supplier_name or sinv.supplier_mobile) if sinv else reason
                txn_label = "Purchase Cancelled"
            ...
            # ── FIFO IN / OUT ────────────────────────────────────
            if qty > 0:
                # ── Rate fallback for zero-rate IN txns ───────────────────
                ...
                # ── FIFO lot vs audit-only IN txn ─────────────────────────
                # txns with lot_remaining IS NOT None → real FIFO IN-lot (purchase, opening)
                # txns with lot_remaining IS None     → audit-only IN (sale cancellation reversal)
                #   These restore qty_on_hand but are NOT new FIFO lots.
                #   Adding them to fifo_batches would make them available for future FIFO
                #   consumption which is WRONG — the original lots were already restored.
                is_real_lot = t.lot_remaining is not None

                if is_real_lot:
                    # Use lot_remaining as qty_remaining — this is the actual available stock.
                    # A cancelled lot has lot_remaining=0 even though qty=90 (the original purchase).
                    # Using qty would show 90 as available when it's actually 0 (cancelled).
                    lot_rem = t.lot_remaining if t.lot_remaining is not None else qty
                    fifo_batches.append({
                        "qty_remaining": lot_rem,
                        "purchase_rate": rate,
                        "txn_date":      t.txn_date,
                    })
        ...
        # Use FIFO batch sum for qty_on_hand — authoritative.
        # qty_in_total - qty_out_total is wrong when legacy edit-delta records exist
        # (even after skipping them, the batches are the ground truth).
        qty_on_hand_val = sum(Decimal(str(b["qty_remaining"])) for b in fifo_batches)
        avg_rate = (
        ...
        category_map[cat]["qty_on_hand"] += qty_on_hand_val   # batch-derived
    ...
    category_summary = [
        {
            ...
            "qty_on_hand":     round(float(v["qty_on_hand"]), 4),   # from FIFO batches
...
async def cancelled_invoices_report(
    ...
    return {
        ...
        "total_cash_fy":     sum(r["cash_receipts_fy"] for r in rows),
        "threshold":         float(SFT_THRESHOLD),
    }
