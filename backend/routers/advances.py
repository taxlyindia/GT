...
async def allocate_advance(
    ...
    for alloc in body.allocations:
        ...
        inv.outstanding = max(Decimal("0"), inv.outstanding - alloc.allocated_amount)  # BUG-ADV-02 FIX
        if inv.outstanding <= Decimal("0"):
            inv.payment_status = PaymentStatus.paid
        elif inv.amount_paid > Decimal("0"):
            inv.payment_status = PaymentStatus.partial

    advance.remaining = max(Decimal("0"), advance.remaining - total_alloc)  # BUG guard
    await db.commit()
...
async def list_advances(
    ...
    # Batch-fetch customer names to avoid N+1 queries
    ...
    cust_map = {c.mobile: c.name for c in cust_result.scalars().all()}

    rows = []
    for a in advances:
        cname = getattr(a, "customer_name", None) or cust_map.get(a.customer_mobile, "—")
        rows.append({
            "id":              a.id,
            ...
            "mode":            a.pay_mode.value if hasattr(a.pay_mode, "value") else str(a.pay_mode),
            "notes":           a.notes or "",
        })
...
async def update_advance(
    ...
    if body.pay_mode is not None:
        # BUG-ADV-03 FIX: was incorrectly using PaymentStatus enum for pay_mode
...
async def cancel_advance(
    ...
    for alloc in alloc_result.scalars().all():
        ...
        if invoice and invoice.tenant_id == tenant_id:
            invoice.amount_paid = max(Decimal("0"), invoice.amount_paid - alloc.allocated_amount)  # BUG-ADV-01 FIX: was alloc.amount_allocated
            invoice.outstanding = max(Decimal("0"), invoice.grand_total - invoice.amount_paid)  # BUG-ADV-02 FIX
