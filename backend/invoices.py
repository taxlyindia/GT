# routers/invoices.py  — Sale Invoice CRUD with FIFO stock engine
# ══════════════════════════════════════════════════════════════════
#
# FIFO rules (see utils/fifo.py for full engine documentation):
#
#  CREATE  → fifo_deduct_sale(): consume IN-lots oldest-first, reduce
#             lot_remaining, record sale txn with FIFO-weighted avg cost.
#
#  CANCEL  → fifo_cancel_sale(): direct reversal of original sale.
#             Restores lot_remaining on exactly the consumed lots.
#             Records one cancellation txn. No fresh FIFO walk.
#             Invoice status → cancelled; credit note number returned.
#
#  EDIT    → fifo_edit_sale(): modifies original sale txn IN-PLACE.
#             Returns old qty to FIFO lots, consumes new qty via FIFO walk.
#             No reversal records, no new sale records created.
...
from utils.fifo import fifo_deduct_sale, fifo_cancel_sale, fifo_edit_sale
...
async def edit_invoice(
    ...
    ─────────────────────────────────────
    Edit modifies the original sale transaction IN-PLACE. No reversal records
    are created. No new sale records are created.

    1. For each item: fifo_edit_sale() returns old consumed qty to FIFO lots,
       then re-consumes the new qty via a fresh FIFO walk, and updates the
       original sale StockTransaction record (qty, purchase_rate, date).

    2. InvoiceItem rows are replaced (old deleted, new inserted).

    3. Financial totals fully recomputed (subtotal, GST, grand_total, outstanding).
