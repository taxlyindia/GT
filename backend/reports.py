...
def _parse_reason(reason: str) -> tuple:
    ...
    Returns (kind, lookup_key):
      ...
      ('sale_id', 5)                   — sale (or edited sale) linked to Invoice by id
      ('sale_cancelled_id', 5)         — sale cancellation, original Invoice id
      ('purchase_cancelled', 'INV-01') — purchase cancellation, original invoice_no
      ('purchase_edited', 'INV-01')    — purchase edit, original invoice_no
    ...
    # Purchase — Supplier Invoice {no} (create or edited)
    ...
    # Backwards-compat: old format "Supplier Invoice {no}"
    ...
    # Sale — Invoice ID {id} (possibly with [Edited] or [CANCELLED] suffix)
    ...
    # Cancelled — Invoice {ID or no}  (old format)
    m = re.match(r'Cancelled\s*[\u2014\-]+\s*Invoice\s+(?:ID\s+)?(.+)', r, re.IGNORECASE)
    if m:
        val = m.group(1).strip()
        if val.isdigit():
            return ('sale_cancelled_id', int(val))
        return ('sale_cancelled_id', val)

    # Edit Reversal — Invoice {id}  (old edit format — no longer created but handle legacy)
...
def _dedup_txns(all_txns: list) -> list:
    ...
    With in-place edit (fifo_edit_sale / fifo_edit_purchase_lot), the original
    transaction IS updated — no duplicate records are created, so dedup is
    straightforward.

    With direct-reversal cancel (fifo_cancel_sale / fifo_cancel_purchase),
    one new cancellation txn is added alongside the original. Both must be
    kept (they represent different events: the original sale + its reversal).

    Rules:
    - Sale txns grouped by invoice_id (if set) or parsed sale_id.
    - Cancellation txns are ALWAYS kept individually (unique key = txn.id).
    - Opening/adjustment/manual txns are always kept individually.
    - If multiple sale txns exist for the same invoice (shouldn't happen after
      in-place edit, but handles legacy data), keep only the latest.
    """
    seen: dict = {}
    for t in all_txns:
        ...
        # Cancellation txns must all be kept — they are distinct reversal events
        ...
        elif kind in ('none', 'purchase_adj') or lookup_key is None:
            # Opening/adjustment/manual — each is a distinct lot, keep all
            key = ('unique', t.id, direction)
        else:
    ...
    for stock in stocks:
        ...
        for t in txns:
            ...
            if kind == 'supplier_invno':
                ...
                if kind in ('cancelled_id', 'sale_cancelled_id'):
                    orig = inv_by_id.get(lkey) if isinstance(lkey, int) else inv_by_no.get(lkey)
                else:
                ...
                txn_label = "Sale Cancelled"   # clear label in FIFO report

            elif kind in ('purchase_cancelled',):
